...
sql_file="${project_root}/backend/src/main/resources/sql/system/report/preview/daily_preview_foundation_v1.sql"
remote_runner="${script_dir}/run_daily_preview_sql.sh"
...
for file in "${sql_file}" "${remote_runner}"; do
  if [[ ! -f "${file}" ]]; then
...
bundle_name="daily-preview-ddl-$(date -u +%Y%m%d-%H%M%SZ)"
bundle_key="_deployment/database/${bundle_name}.tar.gz"
bundle_uri="s3://${bucket_name}/${bundle_key}"
policy_name="project-admin-dev-temporary-daily-preview-secret-read"
uploaded=false
...
install -m 0640 "${sql_file}" "${work_dir}/bundle/daily-preview.sql"
...
if [[ "${command_status}" != "Success" ]]; then
  echo "Daily preview DDL failed. Command ID: ${command_id}" >&2
  exit 1
fi

echo "DAILY_PREVIEW_DDL_COMPLETE"
