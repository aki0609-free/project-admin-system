...
従業員の「日払い対象」チェックと契約情報の「支払サイクル」は意味が重複していた。調査時点で`daily_pay_flag`は従業員情報の保存・再表示にしか使われず、日次支払や帳票の対象判定には使われていなかった。

V1では次の仕様へ統一する。

- 日払い対象の正本は`employee_contract.payment_cycle`とする。
- `payment_cycle=DAILY`の従業員だけ、日報から未保存の日次支払候補を生成する。
- `WEEKLY`と`MONTHLY`の従業員は日次支払候補へ自動表示しない。
- 保存済みの`daily_payments`は履歴として維持し、支払サイクル変更だけで非表示・削除しない。
- 月次の前払い額は従来どおり、対象期間内で`PAID`になった`daily_payments.actual_amount`を集計する。
- 画面とAPIから`dailyPayFlag`を廃止する。

AWS既存DBとの互換性のため`employee_payroll_profile.daily_pay_flag`列はV1中は残す。ただし入力には使わず、従業員契約保存時に`payment_cycle=DAILY`かどうかを自動同期する移行列とする。DB列の物理削除はV2の正式マイグレーションで行う。
