...
export const payrollItemTransactionApi = {
  findAll: (
    ...
    get<EmployeePayrollItemTransaction[]>(basePath, {
      params: {
        path: { employeeId },
        query: { targetType, targetCode, targetMonth },
      },
    }),

  create: (employeeId: number, request: EmployeePayrollItemTransactionRequest) =>
    post<EmployeePayrollItemTransaction, EmployeePayrollItemTransactionRequest>(basePath, request, {
  ...
  remove: (employeeId: number, transactionId: number) =>
    del('/api/employees/{employeeId}/payroll-item-transactions/{transactionId}', {
      params: { path: { employeeId, transactionId } },
    }),
}
